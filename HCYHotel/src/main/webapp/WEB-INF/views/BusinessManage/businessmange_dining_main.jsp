...
.st0 {
	fill: #FFFFFF;
					...
					$(function() {
					        ...
					        $.ajax({
					            ...
					            success: function(jsonObj) {
					                ...
					                $.each(jsonObj.data, function(ind, jsonTemp) {
					                    ...
					                    output += "<td>" + jsonTemp.bookingdate + "<input type='hidden' id='bookingcode' name='bookingcode' value='"+jsonTemp.bookingcode+"'></td>";
					    ...
					    $("#cancelbtn").click(function(){
					    	location.href="delete_diningbooking.do?bookingcode="+bookingcode;
					    })
					});
    ...
    <select id="diningselect">
        ...
        <c:forEach var="diningList" items="${diningList}">
            <option value="${diningList.diningcode}">
                <c:out value="${diningList.diningname}"/>
            </option>
        </c:forEach>
    </select>
</div>
    <div style="display: flex; justify-content: center;" id="output">
        <table style="width: 900px; border: 1px solid black;">
									...
									<c:forEach var="diningResList" items="${diningResList }">
										<tr>
											<td><c:out value="${diningResList.bookingcode }" /></td>
											<td><c:out value="${diningResList.id }" /></td>
											<td><c:out value="${diningResList.bookingname }" /></td>
											<td><c:out value="${diningResList.pplcnt }" /></td>
											<td><c:out value="${diningResList.category }" /></td>
											<td><c:out value="${diningResList.bookingdate }" /><input type="hidden" id="bookingcode" name="bookingcode" value="${diningResList.bookingcode }"></td>
											<c:choose>
												<c:when test="${ diningResList.status eq 'Y'}">
													<td><input type="button" class="btn btn-success"
														id="cancelbtn" value="예약취소"></td>
												</c:when>
												<c:otherwise>
								예약가능
								</c:otherwise>
