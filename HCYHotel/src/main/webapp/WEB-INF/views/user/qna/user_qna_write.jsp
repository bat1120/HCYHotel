...
<script>
  ...
  function goTrackEventMenu(menuName)
  {
    ...
    <div id="contents">
        ...
        <div class="about_content inquiries ">
            ...
            <div class="section_content room">
                ...
                <div class="content_inner">
                    <form method="post" enctype="multipart/form-data" name="customerForm">
                        <!-- 문의 내용 -->
						<input type="hidden" name="ask_type" value="객실">
						<input type="hidden" name="user_sex" value=""><!-- 성별 -->
						<input type="hidden" name="user_mail" value="">
						<input type="hidden" name="user_phone" value="">
						<input type="hidden" name="dine_type">
						<input type="hidden" name="feast_purpose">
						<input type="hidden" name="feast_table">
						<input type="hidden" name="feast_use_room">
						<input type="hidden" name="wedd_type">
						<input type="hidden" name="mem_type">
						<input type="hidden" name="room_inq_sec">
                        
                        <div class="cnt_textarea">
                            <dl class="fclear" id="div_rm_inq">
                                <dt class="inner_tit line2">제목</dt>
                                <dd>
                                <textarea name="content" id="title" maxlength="800" placeholder="제목" title="제목란"></textarea>
                         ...
                         <input type="button" value="문의등록" class="btn btn-primary"/>
		...
		<script>
			var $opt = $(".select_mail option").filter(function(){
				return $(this).val() == "";
			});
            var userEmail

            if( $opt.length == 0 ) {
				$(".select_mail option").filter(function(){
					return $(this).val() == "SELF";
				}).attr("selected","selected");
				$('#email_domain').show();
				$('#email_domain').val("")
                userEmail=$('#email_domain').val();
			} else {
				$opt.attr("selected","selected");
				$('#email_domain').val("");
				$('#email_domain').hide();
                userEmail=$(".select_mail").val();
			}

			$("#phone1 option").filter(function(){
				return $(this).val() == "";
			}).attr("selected","selected");
		</script>
